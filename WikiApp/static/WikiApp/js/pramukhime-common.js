...
function scriptChangeCallback(lang, kb, context) {
	...
	document.getElementById('cmdhelp').style.background = "transparent url('"+piresourcebase+"/static/WikiApp/img/"+ icon.iconFile + "') " + (lang == 'english' ? 100 : -1 * icon.x) + "px " + (lang == 'english' ? 100 : -1 * icon.y) + "px no-repeat";
	...
	document.getElementById('pramukhimecharmap').src = piresourcebase +'/static/WikiApp/img/' + pramukhIME.getHelpImage();
    var filename = pramukhIME.getHelp();
    if (filename != '') {
        //document.getElementById('pramukhimehelpdetailed').src = piresourcebase + 'help/' + filename;
		document.getElementById('pramukhimehelpdetailed').src =piresourcebase +'/static/WikiApp/help/' + filename;
    }
...
function showHelp(elem) {
    ...
    showDialog('WikiAudia Type Pad Help');
